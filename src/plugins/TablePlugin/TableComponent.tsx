...
export function TableComponent({ nodeKey, rows: rawRows, theme }: TableComponentProps): JSX.Element | null {
  ...
  const [selectedCellIDs, setSelectedCellIDs] = React.useState<Array<string>>([]);
  const selectedCellSet = React.useMemo<Set<string>>(() => new Set(selectedCellIDs), [selectedCellIDs]);
  ...
  const updateTableNode = React.useCallback(
  ...
  const modifySelectedCells = React.useCallback(
  ...
  const updateCellsByID = React.useCallback(
  ...
  const clearCellsCommand = React.useCallback((): boolean => {
